...
Veja a seção sobre [execução de testes](https://facebook.github.io/create-react-app/docs/running-tests) para obter mais informações.
...
**Nota: Esta é uma operação irreversível. Uma vez que você faz `eject`, não pode voltar atrás!**
...
Você não é obrigado a usar `eject`. O conjunto de recursos cuidadosamente elaborado é adequado para implantações pequenas e médias, e você não deve sentir a obrigação de usar esse recurso. No entanto, entendemos que essa ferramenta não seria útil se você não pudesse personalizá-la quando estiver pronto para isso.
